...
public class MultiObserversAndCompositeDisposables extends AppCompatActivity {
  ...
  public void subscribe1(View view) {
     ...
     */
    compositeDisposable.add(getAnimalObservable().subscribeOn(Schedulers.io())
        ...
        .filter(
            value -> (value.toLowerCase().startsWith("q") || value.toLowerCase().startsWith("b")))
        .subscribeWith(getAnimalObserver()));
  }

  public void subscribe2(View view) {
    compositeDisposable.add(getAnimalAllCapsObservable().subscribeOn(Schedulers.io())
        .observeOn(AndroidSchedulers.mainThread())
